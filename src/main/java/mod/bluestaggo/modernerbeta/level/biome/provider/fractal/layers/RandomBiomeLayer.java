...
public class RandomBiomeLayer extends Layer {
    public static final com.mojang.serialization.MapCodec<RandomBiomeLayer> CODEC = VersionCompat.createMaybeMapCodec(
        instance -> fillLayerFields(instance)
            .and(ExtendedBiomeIds.CODEC.listOf().fieldOf("biomes").forGetter(layer -> layer.biomes))
            .apply(instance, RandomBiomeLayer::new)
    );

    private final List<ExtendedHolder<Biome>> biomes;

    public RandomBiomeLayer(String id, long seed, List<ExtendedHolder<Biome>> biomes) {
        super(id, seed);
        this.biomes = biomes;
    }

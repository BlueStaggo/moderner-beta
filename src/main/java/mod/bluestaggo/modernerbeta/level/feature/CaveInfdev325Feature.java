...
public class CaveInfdev325Feature extends Feature<OreConfiguration> {
    ...
    *///? } else {
    public CaveInfdev325Feature(com.mojang.serialization.Codec<OreConfiguration> configCodec) {
        super(configCodec);
    }
    //? }
    ...
    public boolean place(
        ...
        *///? } else {
        ...
        double x0 = (float)(baseX + 8) + (Mth.sin(radius) * numberOfBlocks) * 0.4375F;
        double x1 = (float)(baseX + 8) - (Mth.sin(radius) * numberOfBlocks) * 0.4375F;
        double z0 = (float)(baseZ + 8) + (Mth.cos(radius) * numberOfBlocks) * 0.4375F;
        double z1 = (float)(baseZ + 8) - (Mth.cos(radius) * numberOfBlocks) * 0.4375F;
        ...
        try (BulkSectionAccess chunkSectionCache = new BulkSectionAccess(level)) {
            for(int block = 0; block <= numberOfBlocks; block++) {
                ...
                double d10 = (double)(Mth.sin(((float)block * 3.141593F) / (float)numberOfBlocks) * sizeMultiplier + 1.0F) * d9 + 1.0D;
                double d11 = (double)(Mth.sin(((float)block * 3.141593F) / (float)numberOfBlocks) * sizeMultiplier + 1.0F) * d9 + 1.0D;

...
public class CaveInfdev325Feature extends Feature<OreFeatureConfig> {
    public CaveInfdev325Feature(Codec<OreFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<OreFeatureConfig> context) {
        ...
        double x0 = (float)(baseX + 8) + (MathHelper.sin(radius) * numberOfBlocks) * 0.4375F;
        double x1 = (float)(baseX + 8) - (MathHelper.sin(radius) * numberOfBlocks) * 0.4375F;
        double z0 = (float)(baseZ + 8) + (MathHelper.cos(radius) * numberOfBlocks) * 0.4375F;
        double z1 = (float)(baseZ + 8) - (MathHelper.cos(radius) * numberOfBlocks) * 0.4375F;
        ...
        try (ChunkSectionCache chunkSectionCache = new ChunkSectionCache(world)) {
            for(int block = 0; block <= numberOfBlocks; block++) {
                ...
                double d10 = (double)(MathHelper.sin(((float)block * 3.141593F) / (float)numberOfBlocks) * sizeMultiplier + 1.0F) * d9 + 1.0D;
                double d11 = (double)(MathHelper.sin(((float)block * 3.141593F) / (float)numberOfBlocks) * sizeMultiplier + 1.0F) * d9 + 1.0D;
                ...
                for(int x = minX; x <= maxX; x++) {
                    for(int y = minY; y <= maxY; y++) {
                        for(int z = minZ; z <= maxZ; z++) {
                            ...
                            // Check bounds
                            if(dX * dX + dY * dY + dZ * dZ >= random.nextDouble() * fuzz + (1D - fuzz)) {
                                continue;
                            }
